...
Trello.prototype.updateCard = function (cardId, params, callback) {
	// this is the generic update card function which support any of the key which the Trello API support
	// params is an associative array which can contain the following keys [name, desc, closed, idList, due]
    var query = this.createQuery();
		if (params.name) {query.name = name};
    if (params.desc) {query.desc = desc};
    if (params.closed) {query.closed = closed};
    if (params.idList) {query.idList = idList};
    if (params.due) {query.due = due};
